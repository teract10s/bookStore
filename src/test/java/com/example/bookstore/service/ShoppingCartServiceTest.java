...
class ShoppingCartServiceTest {
    ...
    void getShoppingCartByUserId_ValidUser_ReturnDto() {
        ...
        when(userDetailsService.loadUserByUsername(DEFAULT_USER.getEmail()))
                .thenReturn(DEFAULT_USER);
        when(shoppingCartRepository.findByUserId(DEFAULT_USER.getId()))
                .thenReturn(DEFAULT_SHOPPING_CART);
    ...
    void addCartItemToShoppingCart_AddNewCartItem_Success() {
        ...
        when(cartItemRepository
                .findByShoppingCartIdAndBookId(DEFAULT_SHOPPING_CART.getId(), requestDto.bookId()))
        ...
        CartItemDto actual = shoppingCartService
                .addCartItemToShoppingCart(requestDto, authentication);
        assertThat(actual)
                .hasFieldOrPropertyWithValue("id", 1L)
    ...
    void updateQuantityOfCartItem_UpdateQuantityInExistentCartItem_Success() {
        ...
        when(userDetailsService.loadUserByUsername(DEFAULT_USER.getEmail()))
                .thenReturn(DEFAULT_USER);
        when(cartItemRepository.findCartItemByIdAndShoppingCartId(cartItemFromDB.getId(),
                DEFAULT_SHOPPING_CART.getId()))
                .thenReturn(Optional.of(cartItemFromDB));
        when(cartItemRepository.save(any())).thenReturn(updatedCartItem);

        CartItemDto actual = shoppingCartService
                .updateQuantityOfCartItem(authentication, cartItemFromDB.getId(), quantity);
        assertThat(actual)
                .hasFieldOrPropertyWithValue("id", 1L)
